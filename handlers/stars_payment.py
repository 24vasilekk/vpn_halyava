...
async def successful_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, db):
    ...
    if payment and payment[5] == 'pending':
        ...
        if existing_sub:
            ...
            print(f"DEBUG: Current end: {current_end_str}")
            ...
            print(f"DEBUG: New end: {new_end.isoformat()}")
            
            # Обновляем дату окончания
            db.cursor.execute('''
                UPDATE subscriptions
                SET end_date = ?, is_trial = 0
                WHERE user_id = ? AND is_active = 1
            ''', (new_end.isoformat(), user_id))
            db.connection.commit()
            
            print(f"DEBUG: SQL выполнен, подписка обновлена")
        else:
            ...
            if vpn_key and user_uuid:
                ...
                print(f"DEBUG: Новая подписка создана")
        
        # Обновляем статус платежа
        db.update_payment_status(payload, 'paid')
        print(f"DEBUG: Статус платежа обновлен на paid")
